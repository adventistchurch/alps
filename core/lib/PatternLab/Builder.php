...
class Builder {
  ...
  */
  protected function loadMustachePatternLoaderInstance() {
    $this->mpl = new Engine(array(
            "loader" => new PatternLoader(__DIR__.$this->sp,array("patternPaths" => $this->patternPaths), $this),
            "partials_loader" => new PatternLoader(__DIR__.$this->sp,array("patternPaths" => $this->patternPaths), $this)
    ));
  }
  ...
  */
  protected function renderPattern($f,$p) {
    ...
    $d = $this->d;
  //  if (!empty($d['organismListingGrid']) && !empty($d['patternSpecific'][$p]['data']['organismListingGrid']['items'])) {
  //    $d['organismListingGrid']['items'] = $d['patternSpecific'][$p]['data']['organismListingGrid']['items'];
  //  }
  //  if (!empty($d['moleculeListingPager']) && !empty($d['patternSpecific'][$p]['data']['moleculeListingPager']['pages'])) {
  //    $d['moleculeListingPager']['pages'] = $d['patternSpecific'][$p]['data']['moleculeListingPager']['pages'];
  //  }

    // if there is page-level-global data make sure to override the default in $this->d
    if (isset($d["pageLevelGlobal"]) && array_key_exists($p, $this->patternTypeMatch)) {
      if (!empty($d["pageLevelGlobal"]["data"]) && $this->patternTypeMatch[$p] === 'pages') {
        $d = array_replace_recursive($d, $d["pageLevelGlobal"]["data"]);
      }
    }
  ...
  *    - $this->d["pageLevelGlobal"] : holds attributes from the page-level global data files
  *
  ...
  */
  protected function gatherData() {
    ...
    if ($this->remoteJson) {
        $this->d = json_decode(file_get_contents($this->remoteJsonUrl), true);
    // otherwise, gather the data from the main source data.json
    } else if (file_exists($this->sd."/_data/_data.json")) {
        $this->d = json_decode(file_get_contents($this->sd."/_data/_data.json"),true);
        $this->jsonLastErrorMsg("_data/_data.json");
    } else {
        print "Missing a required file, source/_data/_data.json. Aborting.\n";
    ...
    $reservedKeys = array("listItems","cacheBuster","link","patternSpecific", "pageLevelGlobal");
    ...
    $this->d["pageLevelGlobal"] = array();

    // gather the data from the page-level global data.json if it exists.
    if (file_exists($this->sd."/_patterns/04-pages/_data.json")) {
      $this->d["pageLevelGlobal"]["data"] = json_decode(file_get_contents($this->sd."/_patterns/04-pages/_data.json"),true);
      $this->jsonLastErrorMsg("_patterns/04-pages/_data.json");
    }
  }
    ...
    foreach($patternObjects as $name => $object) {
      ...
      if ($object->isDir() && ($depth == 0)) {
        ...
        if ($patternFull[0] != "_") {
          ...
          $this->patternTypeMatch[$patternPartial] = $patternTypeDash;
